...
class ControllerSaleOrder extends Controller {
	...
	public function info() {
		...
		if ($order_info) {
			...
			$order_address = $this->model_sale_order->getOrderShippingAddress($this->request->get['order_id']);
			$data["order_addresses"] = array();
			$this->load->model('tool/upload');

			foreach ($order_address as $adresso) {

				if ($adresso['shipping_address_format']) {
					$format = $adresso['shipping_address_format'];
				} else {
					$format = '{firstname} {lastname}' . "\n" . '{company}' . "\n" . '{address_1}' . "\n" . '{address_2}' . "\n" . '{city} {postcode}' . "\n" . '{zone}' . "\n" . '{country}';
				...
				$replace = array(
					'firstname' => $adresso['shipping_firstname'],
					'lastname'  => $adresso['shipping_lastname'],
					'company'   => $adresso['shipping_company'],
					'address_1' => $adresso['shipping_address_1'],
					'address_2' => $adresso['shipping_address_2'],
					'city'      => $adresso['shipping_city'],
					'postcode'  => $adresso['shipping_postcode'],
					'zone'      => $adresso['shipping_zone'],
					'zone_code' => $adresso['shipping_zone_code'],
					'country'   => $adresso['shipping_country']
				);

				$adresso["shipping_address"] = str_replace(array("\r\n", "\r", "\n"), '<br />', preg_replace(array("/\s\s+/", "/\r\r+/", "/\n\n+/"), '<br />', trim(str_replace($find, $replace, $format))));


				$order_product = $this->model_sale_order->getOrderProduct($this->request->get['order_id'],$adresso["order_product_id"]);

				$option_data = array();

				$options = $this->model_sale_order->getOrderOptions($this->request->get['order_id'], $adresso['order_product_id']);
				...
				$product = array(
					...
					'price'    		   => $this->currency->format($order_product['price'] + ($this->config->get('config_tax') ? $order_info['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']),
					'total'    		   => $this->currency->format($order_product['total'] + ($this->config->get('config_tax') ? ($order_info['tax'] * $order_product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value']),
					'href'     		   => $this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $order_product['product_id'], true)
				);
				 $adresso["product"] = $product;
				$data["order_addresses"][] = $adresso;
			}
			//echo json_encode($data["order_addresses"]);
			// Shipping Address
			...
			*/
			...
			$this->response->setOutput($this->load->view('sale/order_info_multiple', $data));
			}else{
			$this->load->language('sale/order');
